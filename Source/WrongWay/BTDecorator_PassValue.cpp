...
#include "BTDecorator_PassValue.h"
...
/*UBTDecorator_PassValue::UBTDecorator_PassValue(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	NodeName = "Blackboard Based Condition Test";
	//NotifyObserver = EBTBlackboardRestart::ResultChange;
}*/

bool UBTDecorator_PassValue::CalculateRawConditionValue(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory) const
{
...
bool UBTDecorator_PassValue::EvaluateOnBlackboard(const UBlackboardComponent& BlackboardComp) const
{
	bool bResult = false;
...
EBlackboardNotificationResult UBTDecorator_PassValue::OnBlackboardKeyValueChange(const UBlackboardComponent& Blackboard, FBlackboard::FKey ChangedKeyID)
{
...
void UBTDecorator_PassValue::DescribeRuntimeValues(const UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, EBTDescriptionVerbosity::Type Verbosity, TArray<FString>& Values) const
{
...
FString UBTDecorator_PassValue::GetStaticDescription() const
{
...
void UBTDecorator_PassValue::BuildDescription()
{
...
void UBTDecorator_PassValue::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (ChangedPropName == GET_MEMBER_NAME_CHECKED(UBTDecorator_PassValue, BlackboardKey.SelectedKeyName))
	{
	...
	if (ChangedPropName == GET_MEMBER_NAME_CHECKED(UBTDecorator_PassValue, BasicOperation))
	{
	...
	else if (ChangedPropName == GET_MEMBER_NAME_CHECKED(UBTDecorator_PassValue, ArithmeticOperation))
	{
	...
	else if (ChangedPropName == GET_MEMBER_NAME_CHECKED(UBTDecorator_PassValue, TextOperation))
	{
...
void UBTDecorator_PassValue::InitializeFromAsset(UBehaviorTree& Asset)
{
