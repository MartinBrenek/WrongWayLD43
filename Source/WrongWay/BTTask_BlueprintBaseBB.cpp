...
#include "BTTask_BlueprintBaseBB.h"
#include "AIController.h"
#include "BehaviorTree/BTFunctionLibrary.h"
#include "BlueprintNodeHelpers.h"
#include "BehaviorTree/BehaviorTree.h"

UBTTask_BlueprintBaseBB::UBTTask_BlueprintBaseBB(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	UClass* StopAtClass = UBTTask_BlueprintBaseBB::StaticClass();
...
void UBTTask_BlueprintBaseBB::InitializeFromAsset(UBehaviorTree& Asset)
{
...
void UBTTask_BlueprintBaseBB::SetOwner(AActor* InActorOwner) 
{ 
	ActorOwner = InActorOwner;
	AIOwner = Cast<AAIController>(InActorOwner);
}

EBTNodeResult::Type UBTTask_BlueprintBaseBB::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
...
EBTNodeResult::Type UBTTask_BlueprintBaseBB::AbortTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
...
void UBTTask_BlueprintBaseBB::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
...
void UBTTask_BlueprintBaseBB::OnTaskFinished(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, EBTNodeResult::Type TaskResult)
{
...
void UBTTask_BlueprintBaseBB::FinishExecute(bool bSuccess)
{
...
void UBTTask_BlueprintBaseBB::FinishAbort()
{
	UBehaviorTreeComponent* OwnerComp = Cast<UBehaviorTreeComponent>(GetOuter());
...
bool UBTTask_BlueprintBaseBB::IsTaskExecuting() const
{
	UBehaviorTreeComponent* OwnerComp = Cast<UBehaviorTreeComponent>(GetOuter());
...
bool UBTTask_BlueprintBaseBB::IsTaskAborting() const
{
	// use already cached data
	return bIsAborting;
}

void UBTTask_BlueprintBaseBB::SetFinishOnMessage(FName MessageName)
{
	UBehaviorTreeComponent* OwnerComp = Cast<UBehaviorTreeComponent>(GetOuter());
	if (OwnerComp)
	{
...
void UBTTask_BlueprintBaseBB::SetFinishOnMessageWithId(FName MessageName, int32 RequestID)
{
	UBehaviorTreeComponent* OwnerComp = Cast<UBehaviorTreeComponent>(GetOuter());
	if (OwnerComp)
	{
...
FString UBTTask_BlueprintBaseBB::GetStaticDescription() const
{
	FString ReturnDesc = Super::GetStaticDescription();

	UBTTask_BlueprintBaseBB* CDO = (UBTTask_BlueprintBaseBB*)(GetClass()->GetDefaultObject());
...
void UBTTask_BlueprintBaseBB::DescribeRuntimeValues(const UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, EBTDescriptionVerbosity::Type Verbosity, TArray<FString>& Values) const
{
	UBTTask_BlueprintBaseBB* CDO = (UBTTask_BlueprintBaseBB*)(GetClass()->GetDefaultObject());
...
void UBTTask_BlueprintBaseBB::OnInstanceDestroyed(UBehaviorTreeComponent& OwnerComp)
{
	// force dropping all pending latent actions associated with this blueprint
	BlueprintNodeHelpers::AbortLatentActions(OwnerComp, *this);
}
...
bool UBTTask_BlueprintBaseBB::UsesBlueprint() const
{
	return true;
}
